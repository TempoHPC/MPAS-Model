...
nvhpc-scorep:   # BUILDTARGET NVIDIA HPC SDK
	( $(MAKE) all \
	"FC_PARALLEL = scorep mpifort" \
	"CC_PARALLEL = scorep mpicc" \
	"CXX_PARALLEL = scorep mpic++" \
	"FC_SERIAL = nvfortran" \
	"CC_SERIAL = nvc" \
	"CXX_SERIAL = nvc++" \
	"FFLAGS_PROMOTION = -r8" \
...
gfortran-scorep:   # BUILDTARGET GNU Fortran, C, and C++ compilers
	( $(MAKE) all \
	"FC_PARALLEL = scorep mpif90" \
	"CC_PARALLEL = scorep mpicc" \
	"CXX_PARALLEL = scorep mpicxx" \
	"FC_SERIAL = gfortran" \
	"CC_SERIAL = gcc" \
	"CXX_SERIAL = g++" \
	"FFLAGS_PROMOTION = -fdefault-real-8 -fdefault-double-8" \
